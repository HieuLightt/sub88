...
if($dich_vu=="sub-speed"){$dtb="sub_speed"; $action=$dichvu1['5']; $title="Tăng sub nhanh Facebook";}
...
elseif($dich_vu=="sub-sale"){$dtb="sub_sale"; $action=$dichvu1['5']; $title="Follow sale";}
...
<div class="card-body">
  <form submit-ajax="NTD" action="<?=BASE_URL('api/facebook/buy-sub-qualyti');?>" method="post" api_token="<?=$NTD->getUsers('token');?>">
...
<h5 class="modal-title" id="modalSystemLabel">Thông Báo</h5>
...
<button type="button" class="btn btn-danger" data-bs-dismiss="modal">Tôi đã đọc</button>
</div>
</div>
</div>
</div>
