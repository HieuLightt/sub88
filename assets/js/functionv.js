/*! jQuery v3.5.1 | (c) JS Foundation and other contributors | jquery.org/license */
        !function (e, t) {
            ...
            var jt = /%20/g, qt = /#.*$/, Lt = /([?&])_=[^&]*/, Ht = /^(.*?):[ \t]*([^\r\n]*)$/gm,
                Ot = /^(?:GET|HEAD)$/, Pt = /^\/\//, Rt = {}, Mt = {}, It = "*/".concat("*"), Wt = E.createElement("a");
 ...
 type_menu = 'follow';
            function checkCoin() {
                // let package = $('input[name="package"]');

                let prices = getDataAtt('package_name', 'price');
                let package_name = getValueRadioByName('package_name');
                let quantity = getValueById('quantity');
                if (package_name === '30_1') {
                    $('#group_reaction').css('display', 'block');
                } else {
                    $('#group_reaction').css('display', 'none');
                }
                let check_out_coin = parseFloat(prices) * parseFloat(quantity);
                console.log(prices);
                console.log(quantity);
                add_txt_price_per(prices);
                add_check_out_coin(check_out_coin);
            }
    ...
    $(document).ready(function () {
        $(window).resize(function () {
            let $height = $('.topnav').height()
            let $width = $(window).width()
            if ($width >= 992) {
                $('.content-page').css('padding-top', $height)
            } else {
        ...
        $('#modal_notify_system').modal('toggle');
        $(window).bind("beforeunload", function () {
            loading();
            window.localStorage.setItem('uid', uid);
        });

        $('#c_' + category_active).addClass('active');
    });
    ...
    function buyOrder() {
        Swal.fire({
            title: 'Bạn chắc chứ ?',
            text: "Bạn đã đọc hết lưu ý và đã kiểm tra lại đơn hàng ?? ",
            icon: 'warning',
    ...
    function convertUid(provider = 'facebook') {
        ...
        let package_name = getValueRadioByName('package_name');
        if (package_name !== '30_3' && package_name !== '30_8') {
            if (provider === 'facebook') {
                if (isNaN(parseInt(uid)) && uid.length > 0) {
                    window.localStorage.setItem('uid', uid);
    ...
    function convertUidFacebook(link, type, name = 'id') {
        $('input[name="' + name + '"]').val('');
        $('input[name="' + name + '"]').attr('placeholder', "Đang xử lý");
        $.ajax({
            "url": "https://sub48.vn/api/convert-uid",
            "method": "POST",
            "timeout": 0,
            "headers": {
            ...
            "data": JSON.stringify({
                "type": type,
                "api_key_agency": '420bb67c436d02a3eaadZGF0MDgwMWRhdDA4ZGF0MDgwMTAx',
                "link": link
            }),
            success: function (data) {
                if (data == 0) {
                   
                    $('input[name="' + name + '"]').attr('placeholder', "Không Thể Lấy ID");
                } else {
    ...
    function removeOrder(id, link) {
        Swal.fire({
            title: "Bạn đồng hủy id <span class='text-danger'>" + id + "</span>",
            html: "Sau khi bạn hủy hệ thống sẽ hoàn lại số lượng chưa chạy trong 24h và trừ <span class='text-danger'>" + '5,000' + "</span> phí dịch vụ",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
    ...
    function warrantyOrder(id, link) {
        Swal.fire({
            title: "Bạn đồng bảo hành id <span class='text-danger'>" + id + "</span>",
            html: "Bạn đang thực hiện hành động yêu cầu bảo hành cho đơn hàng " + id + " này . đơn hàng sẽ được xử lý bảo hành sau 1 > 3 ngày . bạn có muốn tiếp tục ?",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
    ...
    function checkOrder(id, link) {
        Swal.fire({
            title: "Bạn đồng ý kiểm tra đơn hàng <span class='text-danger'>" + id + "</span>",
            html: "Bạn đang yêu cầu thông tin trạng thái đơn hàng " + id + " , bạn có muốn tiếp tục ?",
            icon: 'warning',
            showCancelButton: true,
            confirmButtonColor: '#3085d6',
            cancelButtonColor: '#d33',
    ...
    function swalError(title, html, link) {
        Swal.fire({
            title: title,
            html: html,
            icon: 'warning',
    ...
    function changePackageNameV2() {
        checkCoin();
        let id = $('#notes_for_customer');
        let notes_for_customer_name = $('#notes_for_customer_name');
        let notes = getDataAtt('package_name', 'notes');
        let name = getDataAtt('package_name', 'name');
        id.empty();
        notes_for_customer_name.empty();
        id.append(notes);
        notes_for_customer_name.append(name);
    }
